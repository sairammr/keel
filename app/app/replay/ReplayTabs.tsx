  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="panel p-3 flex flex-wrap items-center gap-3">
        ...
        <div className="flex">
          <Tab active={source === "engine"} onClick={() => setSource("engine")}>
            engine (live)
          </Tab>
          <Tab active={source === "chain"} disabled={!chain} onClick={() => chain && setSource("chain")}>
        ...
        <span className="text-[12px] text-[color:var(--color-muted)]">
          {onChain
            ...
            : "Both defenders driven through the same market by the real controller + ContractMirror, in-browser."}
        </span>
      </div>
  ...
  return (
    <div className="panel p-4 flex flex-col gap-4">
      <div className="flex items-baseline justify-between flex-wrap gap-2">
        <div>
          <span className="mono text-[15px] font-semibold text-[color:var(--color-keel)]">KEEL</span>
        ...
        <span
          ...
          style={{
            borderColor: consistent ? "var(--color-keel)" : "var(--color-danger)",
            color: consistent ? "var(--color-keel)" : "var(--color-danger)",
          }}
        >
        ...
        <table className="w-full mono text-[12px]">
          <thead>
            <tr className="eyebrow text-left border-b hairline">
              <th className="py-1.5 pr-3">rnd</th>
              ...
              <th className="py-1.5 pr-3 text-right">tx</th>
            </tr>
          </thead>
          <tbody>
            {run.ticks.map((t) => (
              <tr key={t.round} className="border-b hairline last:border-0">
                <td className="py-1.5 pr-3">{String(t.round).padStart(2, "0")}</td>
                <td className="py-1.5 pr-3">${t.priceUsd.toFixed(0)}</td>
                <td className="py-1.5 pr-3">{(t.hfBp / 10000).toFixed(3)}</td>
                <td className="py-1.5 pr-3 text-[color:var(--color-keel)]">{(t.armBp / 10000).toFixed(3)}</td>
                <td className="py-1.5 pr-3 text-[color:var(--color-faint)]">{(t.targetBp / 10000).toFixed(3)}</td>
      ...
      <p className="eyebrow">
        dashed line = the trigger, revealed on-chain and re-derived by the verifier · every
        action row links to its real Sepolia transaction
      </p>
    </div>
  );
}
